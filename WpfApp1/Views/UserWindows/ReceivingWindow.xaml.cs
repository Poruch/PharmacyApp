...
public partial class ReceivingWindow : Window, INotifyPropertyChanged
{
    ...
    public ReceivingWindow(IReceivingService receivingService, IItemService itemService)
    {
        ...
        ClearCodesCommand = new RelayCommand(_ => ScannedCodes.Clear());
        SaveBatchCommand = new RelayCommand(_ => SaveBatch(), _ => CanSave());
    ...
    public string SupplierInfo => FoundSupplier != null
        ? $"{FoundSupplier.Name} (ИНН: {FoundSupplier.Inn})"
        : "Поставщик не найден";
    ...
    private void FindSupplier()
    {
        ...
        if (FoundSupplier == null)
            MessageBox.Show("Поставщик по данной накладной не найден.", "Информация",
    ...
    private bool CanSave() =>
        ...
        ExpiryDate.HasValue && PurchasePrice.HasValue && RetailPrice.HasValue && Quantity > 0;
    ...
    private void SaveBatch()
    {
        try
        {
            ...
            MessageBox.Show("Партия и маркированные экземпляры сохранены.", "Успех",
                MessageBoxButton.OK, MessageBoxImage.Information);
            DialogResult = true;
            Close();
        }
